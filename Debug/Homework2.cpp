...
int main(void)
{
	...
	bg.height = 1280;
	bg.width = 1280;
	bg.tiles[0] = floortile1Tex;
	bg.tiles[1] = dirtbg1Tex;
	bg.tiles[2] = backgroundTex;
	...
	while (!shouldExit) {
		...
		for (r = start_x; r < end_x; r++) {
			...
			for (d = start_y; d < end_y; d++) {
				glDrawSprite(tileIndex[r][d], -camera.x + r * TILE_SIZE, -camera.y + d * TILE_SIZE, TILE_SIZE, TILE_SIZE);
			}
		}
